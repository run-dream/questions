...
1. 数据位置发生变化的时候，InnoDB 表只需要修改主键索引，而内存表需要修改所有索引
2. 内存表不支持 Blob 和 Text 字段，并且即使定义了 varchar(N)，实际也当作 char(N)，也就是固定长度字符串来存储
3. 断电会丢失
4. 锁粒度问题 至于表锁
...
3. 用 select … into outfile 的方法是最灵活的，支持所有的 SQL 写法。但，这个方法的缺点之一就是，每次只能导出一张表的数据，而且表结构也需要另外的语句单独备份。后两种方式都是逻辑备份方式，是可以跨引擎使用的。



### MySQL 权限管理

关键词: ***flush privileges***
...
关键词： ***.ibd*** ***open_files_limit***

回答：

分区表是一个.frm文件对应多个.ibd文件，每个分区对应一个.ibd文件。

在server层上是一个表，但在引擎层是多个表。导致进行DDL操作时，会产生MDL锁
...
- **FULL JOIN**：只要其中一个表中存在匹配，则返回行
  ...
  left join 不一定是左表驱动右表，如果where条件中被驱动表的字段放在 where 条件里面做等值判断或不等值判断就会右表驱动左表
...
|           | 作用                           | 实现     | 长度                                              |
| --------- | ------------------------------ | -------- | ------------------------------------------------- |
| trx_id    | MVVM 隔离性                    | innodb   | 8个字节                                           |
| gtid      | 主从复制                       | Server层 | server_uuid:gno                                   |
| xid       | 两阶段提交 关联redolog和binlog | Server层 | 8个字节                                           |
| thread_id | 线程 id                        | Server层 | 4个字节                                           |
| row_id    | 表主键                         |          | 自己定义的int,long，系统实现的8位但只用了6 个字节 |
